...
const collectionFactory = url => name => {
  const collectionUrl = `${url}/api/${name}`
  ...
  const full = {
    ...
    findByIdAndDelete: id => deleteData(`${collectionUrl}/${id}`, {}),
    findByIdAndUpdate: (id, data) => putData(`${collectionUrl}/${id}`, data),
  }
  console.info(full)
  return full
}
const getClient = (url, options) => {
  ...
  console.info('client opts', options)
  const collection = collectionFactory(url)
  return {
    collection,
  }
}
const clientFromArgs = function(args) {
  let db = getClient()

  console.info('return client?')
  return db
}
...
class Model {
  ...
  static async connect(connection, options = {}, name = 'default') {
    ...
    const client = getClient(connection.uri, options)

    Model.clients[name] = client
    Model.dbs[name] = client.db(connection.db)

    return Model.dbs[name]
  }
  ...
  static disconnect(name) {
    if (name === undefined) {
      Object.keys(Model.dbs).forEach(key => {
        delete Model.dbs[key]

        Model.clients[key].close()
      })

      return
    }

    if (!Model.dbs.hasOwnProperty(name)) {
      throw new Error(`Db connection '${name}' not found.`)
    }

    delete Model.dbs[name]

    Model.clients[name].close()
  }
  ...
  static validate(input) {
    return Validation.validate(input, this.schema)
  }

  validate() {
    return Validation.validate(this, this.constructor.schema)
  }
  ...
  static with(name) {
    ...
    const db = Model.dbs[name]
    const boundFunctionsId = `__ModelDbBound${this.name}__`
